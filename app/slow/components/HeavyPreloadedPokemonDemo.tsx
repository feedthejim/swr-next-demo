...
export function HeavyPreloadedPokemonDemo() {
  const {
    data: preloadedData,
    isLoading,
  } = usePreloadedSWR(heavyPokemon, { generations: [1] });
  ...
  // Display first 12 Pokemon
  const displayedPokemon = preloadedData?.slice(0, 12) || [];
      ...
      <div className="space-y-3 flex-1">
        <div className="flex justify-between items-center">
          <div className="text-sm text-gray-300">
            ✅ Showing {displayedPokemon.length} of {preloadedData?.length || 0} Pokemon
          </div>
        </div>
